...
for k in range(1, outer_epoch+1):
    ...
    omega_old = omega
    D10.append(omega)
    S10.append(np.linalg.norm(holder.logistic_indiv_grad(omega), ord=2)**2)
    SS10.append(np.linalg.norm(holder.logistic_indiv_grad(w_ref), ord=2)**2)
    v = holder.logistic_indiv_grad(w_ref)
    omega = omega_old - eta_0 * v
    D10.append(omega)
    S10.append(np.linalg.norm(holder.logistic_indiv_grad(omega), ord=2)**2)
...
for k in range(1, outer_epoch+1):
    ...
    omega_old = omega
    D11.append(omega)
    S11.append(np.linalg.norm(holder.logistic_indiv_grad(w_ref), ord=2)**2)
    SS11.append(np.linalg.norm(holder.logistic_indiv_grad(omega), ord=2)**2)
    v = holder.logistic_indiv_grad(w_ref)
    omega = omega_old - eta_0 * v
    D11.append(omega)
    S11.append(np.linalg.norm(holder.logistic_indiv_grad(omega), ord=2)**2)
    for j in range(1, inner_loop+1):
        batch_b = random.sample(list(range(holder.num)), size_b)
        v = holder.logistic_indiv_grad(omega, batch_b) - holder.logistic_indiv_grad(omega_old, batch_b) + v
        batch_bh = random.sample(list(range(holder.num)), size_bh)
...
pass1 = 1.036*np.array([int(i) for i in range(1, 21)])
pass2 = 1.036*np.array([int(i) for i in range(1, 21)])
pass3 = 1.01*np.array([int(i) for i in range(1, 21)])
pass4 = 1*np.array([int(i) for i in range(1, 21)])
pass5 = 1.014*np.array([int(i) for i in range(1, 21)])
pass6 = 1.88 * np.array([int(i) for i in range(1, 16)])
pass7 = 1.88 * np.array([int(i) for i in range(1, 16)])
pass8 = 1.012*np.array([int(i) for i in range(1, 21)])
pass9 = 1.012*np.array([int(i) for i in range(1, 21)])
pass10 = 1.036*np.array([int(i) for i in range(1, 21)])
pass11 = 1.036*np.array([int(i) for i in range(1, 21)])
plt.xlim(0, 16)
plt.xticks(range(0, 16, 3))
plt.ylim(10**(-14), 10**(-2))
line1, = plt.semilogy(pass1, SS1, linestyle='-', linewidth=2, color='black', label=r'mS2GD-RHBB(3)-pure')
line2, = plt.semilogy(pass2, SS2, linestyle='-', linewidth=2, color='lightpink', label=r'MB-SARAH-RHBB(3)-pure')
