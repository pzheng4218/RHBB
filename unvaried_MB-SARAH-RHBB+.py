...
def data_generate(n, d, input_file="", output_file=" ",):
    ...
    for row in x_train:
        for b in range(1, d + 1):
            row.setdefault(b, 0)
        temp.append([row[b] for b in range(1, d+1)])
    x_train = np.array(temp, dtype='float32').reshape(n, d)
    y_train = np.array(y_train, dtype='float32')
    f = open(output_file, 'wb')
    pickle.dump([x_train, y_train], f)
    f.close()
data_generate(1000, 24, input_file=r"D:\libsvmdataset\german.txt", output_file=r"D:\dataset\german.txt")
...
line1, = plt.semilogy(pass1, SS1, linestyle='--', linewidth=2.5, color='brown', label=r'MB-SARAH-RBB')
line2, = plt.semilogy(pass1, SS2, linestyle='-', linewidth=2.5, color='lightpink', label=r'MB-SARAH-RBB+')
line3, = plt.semilogy(pass1, SS3, linestyle='--', linewidth=2.5, color='cyan', label=r'MB-SARAH-RHBB(3)')
line4, = plt.semilogy(pass1, SS4, linestyle='-', linewidth=2.5, color='mediumpurple', label=r'MB-SARAH-RHBB(3)+')
line5, = plt.semilogy(pass1, SS5, linestyle='--', linewidth=2.5, color='silver', label=r'MB-SARAH-RHBB(6)')
line6, = plt.semilogy(pass1, SS6, linestyle='-', linewidth=2.5, color='darkgreen', label=r'MB-SARAH-RHBB(6)+')
line7, = plt.semilogy(pass1, SS7, linestyle='--', linewidth=2.5, color='dodgerblue', label=r'MB-SARAH-RHBB(8)')
line8, = plt.semilogy(pass1, SS8, linestyle='-', linewidth=2.5, color='plum', label=r'MB-SARAH-RHBB(8)+')
font1 = {'size': 7}
plt.legend(handles=[line1, line2, line3, line4, line5, line6, line7, line8], prop=font1)
# plt.savefig('australian_MB-holder-RHBB+_I.eps', dpi=600, format='eps')
plt.show()
