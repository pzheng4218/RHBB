...
def data_generate(n, d, input_file="", output_file=" ",):
    ...
    f.close()
# data_generate(8124, 112, input_file=r"D:\libsvmdataset\rcv1.txt", output_file=r"D:\dataset\rcv1.txt")
...
class DataHolder:
    ...
    def logistic_indiv_function(self, w, batch=None):
        ...
        u = np.sum(np.log(np.exp(-data.dot(w) * target) + 1)) / len(batch)
        return u
...
lam = 10**(-2)
holder = DataHolder('australian', 0, lam)
inner_loop = 35
outer_epoch = 20
omega = 0
eta_0 = 0.1
...
#     p1 += np.linalg.norm(holder.train_set[0][j], ord=np.inf)
# for j in range(holder.num):
...
for k in range(1, outer_epoch+1):
    ...
    for j in range(1, inner_loop+1):
        ...
        ita1 = (1 / size_bh) * gamma2 * (np.linalg.norm(s, ord=2)**2) / s.dot(y)
...
for k in range(1, outer_epoch+1):
    ...
    for j in range(1, inner_loop+1):
        ...
        v = holder.logistic_indiv_grad(omega, batch_b) - holder.logistic_indiv_grad(w_ref, batch_b) + phi
        s = omega - omega_old
        y = holder.logistic_indiv_grad2(omega, p_list, batch_bh) - holder.logistic_indiv_grad2(omega_old, p_list, batch_bh)
        ita1 = (1 / size_bh) * gamma2 * (np.linalg.norm(s, ord=2)**2) / s.dot(y)
        # ita2 = (1 / size_bh) * gamma * (s.dot(y)) / (np.linalg.norm(y, ord=2) ** 2)
        # ita = np.sqrt(ita1*ita2)
        # ita = ita1 * 0.8 + ita2 * 0.2
...
for k in range(1, outer_epoch+1):
    ...
    for j in range(1, inner_loop+1):
        ...
        ita1 = (1 / max(size_bh, size_bh2)) * gamma2 * (np.linalg.norm(s, ord=2) ** 2) / s.dot(y)
        ita2 = (1 / max(size_bh, size_bh2)) * gamma2 * (s.dot(y2)) / (np.linalg.norm(y2, ord=2) ** 2)
        # ita = np.sqrt(ita1*ita2)
        ita = ita1 * 3 + ita2 * (-2)
        omega_old = omega
...
# 观察在一次外循环中的内循环
plt.figure()
plt.xlabel('Number of Effective Passess')
plt.ylabel(r'$||\nabla P(w)||^2$')
...
plt.ylim(10**(-14), 10**(-2))
pass1 = 1.2*np.array([int(i) for i in range(1, 21)])
pass2 = 1.07*np.array([int(i) for i in range(1, 21)])
pass3 = 1.14*np.array([int(i) for i in range(1, 21)])
line1, = plt.semilogy(pass3, SS1, linestyle='--', linewidth=2.5, color='brown', label=r'mS2GD-RBB')
line2, = plt.semilogy(pass3, SS2, linestyle='-', linewidth=2.5, color='lightpink', label=r'mS2GD-RBB+')
line3, = plt.semilogy(pass3, SS3, linestyle='--', linewidth=2.5, color='cyan', label=r'mS2GD-RHBB(3)')
line4, = plt.semilogy(pass3, SS4, linestyle='-', linewidth=2.5, color='mediumpurple', label=r'mS2GD-RHBB(3)+')
line5, = plt.semilogy(pass3, SS5, linestyle='--', linewidth=2.5, color='silver', label=r'mS2GD-RHBB(6)')
line6, = plt.semilogy(pass3, SS6, linestyle='-', linewidth=2.5, color='steelblue', label=r'mS2GD-RHBB(6)+')
line7, = plt.semilogy(pass3, SS7, linestyle='--', linewidth=2.5, color='tomato', label=r'mS2GD-RHBB(8)')
line8, = plt.semilogy(pass3, SS8, linestyle='-', linewidth=2.5, color='y', label=r'mS2GD-RHBB(8)+')
font1 = {'size': 7}
plt.legend(handles=[line1, line2, line3, line4, line5, line6, line7, line8], prop=font1)
# plt.savefig('australian_mS2GD-RHBB+_I.eps', dpi=600, format='eps')
plt.show()
