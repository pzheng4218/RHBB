...
for k in range(1, outer_epoch+1):
    ...
    v = holder.logistic_indiv_grad(w_ref)
    if k > 1:
        eta = 0.5 * size_b/inner_loop * (np.linalg.norm(W[-1]-W[-2], ord=2)**2/(W[-1]-W[-2]).dot(holder.logistic_indiv_grad(W[-1])-holder.logistic_indiv_grad(W[-2])))
    else:
        eta = eta_0
...
line1, = plt.semilogy(epochs, E1, linestyle='-.', linewidth=2, color='mediumpurple', marker='o', markersize=6, label=r'MB-SARAH-RBB1')
line2, = plt.semilogy(epochs, E2, linestyle='--', linewidth=2, color='turquoise', marker='o', markersize=6, label=r'MB-SARAH-RBB2')
line3, = plt.semilogy(epochs, E3, linestyle='-', linewidth=2, color='peru', marker='*', markersize=6, label=r'MB-SARAH-BB1')
# line4, = plt.semilogy(epochs, E4, linestyle='-', linewidth=2.5, color='mediumpurple', label=r'MB-holder-BB2')
...
plt.grid(True)
# plt.savefig('BB1_RBB_covtype_MB-SARAH.eps', dpi=600, format='eps')
plt.show()
